...
(function () {
  ...
  //  vel   = velocità massima della racchetta (unità/s)
  ...
  var DIFF = {
    facile:    { nome: "Facile",    vel: 0.60, reaz: 0.35, err: 0.25, track: 0.10, cool: 0.80, dy: 0.24 },
    medio:     { nome: "Medio",     vel: 0.85, reaz: 0.25, err: 0.20, track: 0.25, cool: 0.55, dy: 0.22 },
    difficile: { nome: "Difficile", vel: 1.10, reaz: 0.15, err: 0.13, track: 0.40, cool: 0.40, dy: 0.20 }
  };
  // muove la racchetta del bot (in alto: gx,gy)
  function botMuovi(st, dt, D) {
    var b = st._bot || (st._bot = { piano: 0, tx: 0.5, ty: 0.2, ex: 0, cool: 0, lento: 0, modo: "" });
    var md = RP + RPAD;
    // appena tocca il disco si ritira un attimo: così non lo incastra contro il bordo
    if (b.cool <= 0 && Math.hypot(st.px - st.gx, st.py - st.gy) < md + 0.004) { b.cool = D.cool; b.piano = 0; }
    if (b.cool > 0) b.cool -= dt;
    // troppo attaccato al bordo alto il bot non può mettersi "dietro" il disco
    var raggiungibile = st.py >= 0.13;
    ...
    if (b.piano <= 0) {                                  // decide solo ogni "tempo di reazione"
      b.piano = D.reaz;
      var inArrivo = st.pvy < -0.3;                      // tiro che viene verso la sua porta
      var modo = (b.cool > 0 || inArrivo || !raggiungibile || st.py >= ASP / 2 || st.pvy >= 0.6) ? "difendi" : "attacca";
      if (modo !== b.modo) { b.modo = modo; b.ex = (Math.random() - 0.5) * D.err; }
      if (modo === "attacca") {
        // in due tempi: prima prende la rincorsa DIETRO al disco (sulla linea verso la
        // porta avversaria), poi ci passa attraverso e tira
        var vx = 0.5 - st.px, vy = ASP - st.py, vl = Math.hypot(vx, vy) || 1; vx /= vl; vy /= vl;
        var rx = clamp(st.px - vx * md * 1.7, RPAD, 1 - RPAD), ry = clamp(st.py - vy * md * 1.7, RPAD, ASP / 2 - RPAD);
        if (Math.hypot(st.gx - rx, st.gy - ry) < 0.05) { b.tx = st.px + vx * md * 0.5; b.ty = st.py + vy * md * 0.5; }
        else { b.tx = rx; b.ty = ry; }
      } else if (!raggiungibile) {
        b.tx = 0.5; b.ty = 0.2;                          // disco sul bordo: si toglie di mezzo
      } else {
        b.tx = 0.5 + (st.px - 0.5) * D.track + b.ex;     // difesa davanti alla porta
        b.ty = D.dy;
      }
      b.tx = clamp(b.tx, RPAD, 1 - RPAD); b.ty = clamp(b.ty, RPAD, ASP / 2 - RPAD);
    }
    var dx = b.tx - st.gx, dy = b.ty - st.gy, dist = Math.hypot(dx, dy), step = D.vel * dt;
