...
(function () {
  ...
  function assicuraStile() {
    ...
    defs.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg"><defs>' +
      ...
      '<linearGradient id="scgC" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#e2687a"/><stop offset="1" stop-color="#a81f2c"/></linearGradient>' +
      '<linearGradient id="scgS" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#77a9de"/><stop offset="1" stop-color="#215c9c"/></linearGradient>' +
      '<linearGradient id="scgB" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#b07c46"/><stop offset="1" stop-color="#6a4020"/></linearGradient>' +
      simbolo("D") + simbolo("C") + simbolo("S") + simbolo("B") + '</defs></svg>';
    document.body.appendChild(defs);
  }
  // simbolo del seme in un box 10x10 (moneta / coppa / spada / bastone)
  function simbolo(s) {
    var g;
    if (s === "D") g = '<circle cx="5" cy="5" r="4.3" fill="url(#scgD)" stroke="#8a6a1e" stroke-width=".5"/><circle cx="5" cy="5" r="2.6" fill="none" stroke="#8a6a1e" stroke-width=".45"/><path d="M5 3.15l.62 1.26 1.39.2-1.005.98.237 1.385L5 6.36l-1.243.615.237-1.385L2.99 4.61l1.39-.2z" fill="#8a6a1e"/>';
    else if (s === "C") g = '<path d="M2.15 2.05H7.85V3.15A2.85 2.85 0 0 1 2.15 3.15Z" fill="url(#scgC)" stroke="#7c1620" stroke-width=".3"/><rect x="4.5" y="4.15" width="1" height="2.5" fill="#a81f2c"/><path d="M2.85 7.75Q5 6.55 7.15 7.75Q5 8.25 2.85 7.75Z" fill="#a81f2c"/><circle cx="5" cy="1.65" r=".72" fill="#a81f2c"/>';
    else if (s === "S") g = '<path d="M5 .9 5.85 6.2 5 7.5 4.15 6.2Z" fill="url(#scgS)" stroke="#1c4c86" stroke-width=".3"/><rect x="2.7" y="6.05" width="4.6" height=".95" rx=".45" fill="#215c9c"/><rect x="4.6" y="6.9" width=".8" height="1.8" fill="#215c9c"/><circle cx="5" cy="8.95" r=".72" fill="#215c9c"/>';
    else g = '<path d="M4.35 8.9C3.95 6.2 4.15 3.3 5 1.5C5.85 3.3 6.05 6.2 5.65 8.9C5 9.35 5 9.35 4.35 8.9Z" fill="url(#scgB)" stroke="#5a3d20" stroke-width=".4"/>'
      + '<circle cx="5" cy="1.55" r=".7" fill="url(#scgB)" stroke="#5a3d20" stroke-width=".3"/>'
      + '<path d="M4.5 3.7 3.35 3.15" stroke="#5a3d20" stroke-width=".5" stroke-linecap="round"/>'
      + '<path d="M5.55 5 6.75 4.5" stroke="#5a3d20" stroke-width=".5" stroke-linecap="round"/>'
      + '<path d="M4.5 6.3 3.45 5.95" stroke="#5a3d20" stroke-width=".5" stroke-linecap="round"/>';
    return '<symbol id="scSeme' + s + '" viewBox="0 0 10 10">' + g + '</symbol>';
  }
  // posizioni dei "pips" (cx, cy, dimensione) su una carta 100x150, come sulle carte vere
  var PIPS = {
    1: [[50, 75, 42]],
    2: [[50, 46, 27], [50, 104, 27]],
    3: [[50, 40, 24], [50, 75, 24], [50, 110, 24]],
    4: [[36, 48, 24], [64, 48, 24], [36, 102, 24], [64, 102, 24]],
    5: [[36, 47, 22], [64, 47, 22], [50, 75, 23], [36, 103, 22], [64, 103, 22]],
    6: [[36, 44, 22], [64, 44, 22], [36, 75, 22], [64, 75, 22], [36, 106, 22], [64, 106, 22]],
    7: [[36, 42, 20], [64, 42, 20], [50, 58, 20], [36, 75, 20], [64, 75, 20], [36, 108, 20], [64, 108, 20]]
  };
  ...
  function cartaSVG(carta, w) {
    ...
    if (v >= 8) {
      centro = '<rect x="23" y="34" width="54" height="82" rx="7" fill="rgba(0,0,0,.03)" stroke="' + col + '" stroke-width="1.4" opacity=".7"/>' +
        uso(s, 50, 56, 28) +                                                                  // seme GRANDE (ben visibile)
        '<text x="50" y="108" font-size="33" text-anchor="middle">' + FIG[v] + '</text>';     // figura (Fante/Cavallo/Re)
    } else { (PIPS[v] || []).forEach(function (p) { centro += uso(s, p[0], p[1], p[2]); }); }
    function angolo() { return '<text x="8.5" y="20" font-size="18" font-weight="800" fill="' + col + '" font-family="Georgia,serif">' + v + '</text>' + uso(s, 13, 28, 11); }
  ...
  function renderScopa(t, C, cb) {
    ...
    vm.tavolo.forEach(function (c) {
      ...
      tw.appendChild(cartaEl(el, c, 66, extra, (mioTurno && cartaSel && capIds[c.id]) ? function () { C.tapTavolo(c.id); } : null));
    });
