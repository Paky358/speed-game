...
(function () {
  ...
  function renderScopa(t, C, cb) {
    ...
    // carte in tavola: grandezza FISSA, quella con cui 6 carte stanno comode (su una o due file).
    ...
    var wT = misuraPer(6);
    if (Math.floor((tavAw - 15) / 4) >= wT - 3) wT = Math.min(wT, Math.floor((tavAw - 15) / 4));   // se per poco non ci stanno 4 in fila, stringo di un pelo
    if (nT > 6) wT = Math.min(wT, misuraPer(nT));
    wT = Math.max(26, wT);
    // file: quante carte ci stanno in una riga a questa grandezza, poi righe bilanciate (es. 5 = 3+2, 6 = 3+3)
    var maxFila = Math.max(1, Math.floor((tavAw + 5) / (wT + 5))), righeT = Math.ceil(Math.max(1, nT) / maxFila);
    var perRiga = Math.ceil(Math.max(1, nT) / righeT);
