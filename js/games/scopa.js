...
(function () {
  ...
  function assicuraStile() {
    ...
    st.textContent = [
      ...
      ".sc-terra.sc-t3d{position:absolute;left:5%;right:5%;top:17%;bottom:4%;padding:0;transform-style:preserve-3d}",
  ...
  function renderScopa(t, C, cb) {
    ...
    // carte in tavola: grandezza FISSA, quella con cui 6 carte stanno comode (3 posti per fila, 2 file).
    // Da 1 a 6 carte non cambia mai; solo con 7 o più si rimpiccioliscono quanto basta.
    var tavAw = (pianoW - 24) * 0.9 - 8, tavAh = pianoH * 0.78;
    ...
    var wT = Math.min(110, Math.floor((tavAw - 10) / 3), Math.floor((tavAh - 5) / 2 / ASP_CARTA));   // 6 posti: 3 per fila, 2 file
    if (nT > 6) wT = Math.min(wT, misuraPer(nT));
    wT = Math.max(26, wT);
    // POSTI FISSI in tavola: ogni carta resta nel suo posto finché non viene presa (niente carte che "saltano");
    // una carta nuova va nel primo posto libero. Griglia fissa da 6 posti (3 x 2); più posti solo con 7+ carte.
    ...
    var nPosti = Math.max(6, maxPosto);
    var maxFila = Math.max(1, Math.floor((tavAw + 5) / (wT + 5)));
    var colT = nPosti <= 6 ? Math.min(3, maxFila) : Math.min(maxFila, Math.ceil(nPosti / Math.ceil(nPosti / maxFila)));
