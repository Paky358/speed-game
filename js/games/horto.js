...
(function () {
  ...
  var COLORI = ["#ffd43b", "#ff6b6b", "#4dabf7", "#51cf66"];
  ...
  function assicuraStile() {
    ...
    s.textContent = [
      ...
      ".ho-ff-line{position:absolute;top:0;bottom:0;right:16%;width:8px;z-index:1;opacity:.9;",
        "background:repeating-linear-gradient(45deg,#fff 0 6px,#111 6px 12px);}",
      ".ho-ff-row{position:absolute;display:flex;align-items:center;gap:7px;left:-22%;",
        "transition:left 1.9s cubic-bezier(.25,.6,.3,1);z-index:2;}",
      ".ho-ff-row.go{left:var(--x);}",
      ".ho-ff-num{width:19px;height:19px;border-radius:50%;font-size:.7rem;font-weight:900;color:#111;",
  ...
  function costruisci(t, N, nomi, io, cb) {
    ...
    for (var i = 0; i < N; i++) {
      var lane = el("div", { class: "ho-lane" + (i === io ? " mia" : "") });
      lane.appendChild(el("div", { class: "ho-num", style: "background:" + COLORI[i % 4], text: String(i + 1) }));
  ...
  function fotoFinish(t, ord, nomi, io, foto, poi) {
    ...
    for (var k = 0; k < N; k++) {
      ...
      var x = 6 + Math.min(1, foto[k]) * 74;   // distanza VERA all'istante del traguardo (corsia sua)
      var row = el("div", { class: "ho-ff-row" + (vinc ? " win" : ""), style: "top:" + (12 + k * 46) + "px;--x:" + x.toFixed(1) + "%" }, [
    ...
    t.mostra(s);
    setTimeout(function () { rows.forEach(function (r) { r.classList.add("go"); }); }, 80);
    to1 = setTimeout(function () { big.textContent = "🏆 " + nomi[vincitore] + (vincitore === io ? " (tu)" : "") + " vince!"; big.classList.add("show"); traguardoFX(); }, 2100);
    to2 = setTimeout(vai, 4600);
  }
  ...
  function corsa(t, nRivali, diff) {
    ...
    for (var i = 1; i < N; i++) { cav.push(nuovoCav(false)); nomi.push("🤖 " + ["Furia", "Saetta", "Tornado"][i - 1]); }
  ...
  SG.registra({
    ...
    regole: [
      ...
      "Modalità: <b>contro i bot</b> (1–3) oppure <b>online</b> (ognuno dal suo telefono; i posti liberi li giocano i bot)."
    ],
    impostazioni: function (box, dove, aiuti) {
      var el = aiuti.el;
      dove.modo = "bot"; dove.rivali = 3; dove.difficolta = "medio";
      var bBot, bOnl, boxRiv, notaOnline;
      function selM(m) { dove.modo = m; bBot.className = "modo-chip" + (m === "bot" ? " attiva" : ""); bOnl.className = "modo-chip" + (m === "online" ? " attiva" : ""); boxRiv.hidden = (m !== "bot"); notaOnline.hidden = (m !== "online"); }
      ...
      boxRiv = el("div", {});
      boxRiv.appendChild(el("div", { class: "etichetta", style: "margin-top:10px", text: "Quanti avversari (bot)" }));
      ...
      boxRiv.appendChild(wR); box.appendChild(boxRiv);
    ...
    avvia: function (t) {
      ...
      if (imp.modo === "online") return hostHorto(t, imp.difficolta || "medio");
      return corsa(t, imp.rivali || 3, imp.difficolta || "medio");
    }
  });
})();
