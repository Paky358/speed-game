...
(function () {
  ...
  SG.registra({
    ...
    avvia: function (t) {
      ...
      var st = {
        g: t.giocatori.map(function (n) { return { nome: n, punti: 0, voto: null }; }),
        carta: scegli(pool),
        impostore: Math.floor(Math.random() * t.giocatori.length),
        suggerimento: imp.suggerimento !== false,
        iniziante: Math.floor(Math.random() * t.giocatori.length)
      };
      rivela(t, st, 0);
    }
  });
