...
_Aggiornato: 16 settembre 2026 (Palla a Pendolo: lancio a forza fissa, mira di lato e multiplayer online con ruoli in lobby; Horto Muso online più fluido)_
...
- **Gioco 16 — Palla a Pendolo** 🎯 (`js/games/pendolo.js`, id `pendolo`, categoria **Minigiochi**):
  ...
  **online host-autoritativo** (`hostPendolo`/`ospitePendolo`): in **lobby ognuno sceglie il ruolo**
  ...
  o `mov/salta`) e **interpolano** le posizioni (buffer+ritardo, come Glow Hockey). **Sfida a tempo**:
  vince il lanciatore se li butta giù tutti, altrimenti vince chi resta sulla trave (esiti opposti e per
  ruolo). Render condiviso da snapshot (`render(ref, S, dt)`); rilevo le morti lato client per
